...
@section('content')
  <div class="container mx-auto">
      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-2">
        ...
        <li>
          <div class="flex items-center">
            ...
            <span class="ml-1 text-sm font-medium text-gray-600">Edit</span>
          </div>
        </li>
      </ol>
    </nav>
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-800 mb-1">Edit Product</h1>
        <p class="text-gray-500 text-sm">Update product information for {{ $categorySub->name }}</p>
      </div>
    ...
    <section>
      <div class="bg-white border border-gray-200">
        <form action="{{ route('app.products.update', ['id_product' => $product->id]) }}" method="POST"
          ...
          <div class="p-6">
            ...
            <div class="mb-6">
              ...
              <div id="currentImagesContainer" class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                @foreach ($product->image as $index => $image)
                  <div class="relative group current-image-item" data-image-id="{{ $index }}">
                    ...
                    <button type="button" onclick="markImageForDeletion({{ $index }})"
                      class="delete-current-image absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center transition-colors shadow-lg">
            ...
            {{-- <div class="mb-6">
              ...
              <div id="keyPointsContainer" class="space-y-3">
                @if (old('description_point'))
                  @foreach (old('description_point') as $point)
                    <div class="flex gap-2 key-point-item">
                      <input type="text" name="description_point[]" value="{{ $point }}"
                  ...
                  @endforeach
                @elseif($product->description_point && is_array($product->description_point))
                  @foreach ($product->description_point as $point)
                    <div class="flex gap-2 key-point-item">
                      <input type="text" name="description_point[]" value="{{ $point }}"
                        class="flex-1 px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-amber-600 focus:outline-0"
                        placeholder="Enter key point" required>
  ...
  {{-- Scripts --}}
  <script>
    // Store selected files and images to delete
    let selectedFiles = [];
    let imagesToDelete = [];
    ...
    function markImageForDeletion(imageId) {
      ...
      if (imageItem.classList.contains('opacity-50')) {
        // Unmark for deletion
        imageItem.classList.remove('opacity-50');
        imageItem.querySelector('.delete-current-image').classList.remove('bg-green-500', 'hover:bg-green-600');
        imageItem.querySelector('.delete-current-image').classList.add('bg-red-500', 'hover:bg-red-600');
        imagesToDelete = imagesToDelete.filter(id => id !== imageId);
      } else {
        // Mark for deletion
        imageItem.classList.add('opacity-50');
        imageItem.querySelector('.delete-current-image').classList.remove('bg-red-500', 'hover:bg-red-600');
        imageItem.querySelector('.delete-current-image').classList.add('bg-green-500', 'hover:bg-green-600');
